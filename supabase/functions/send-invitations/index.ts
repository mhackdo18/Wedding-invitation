...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { action, test_email, guest_ids, guest_id } = await req.json();
    ...
    const { data: site } = await supabase
      .from("site_settings")
      .select(
        "partner1_name, partner2_name, wedding_date, invitation_wax_seal_color, invitation_wax_seal_image_url, invitation_envelope_color"
      )
      .limit(1)
      .maybeSingle();
