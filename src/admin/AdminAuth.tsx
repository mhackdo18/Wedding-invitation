...
export default function AdminAuth({ section }: { section: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    if (mode === 'register') {
      const { data: collabs } = await supabase.from('collaborators').select('email, role').order('created_at');
      const list = (collabs as Collaborator[]) || [];
      const isInvited = list.some((c) => c.email.toLowerCase() === email.toLowerCase());
      if (list.length > 0 && !isInvited) {
        setError('Registration is closed. Ask an existing admin to invite you before you can create an account.');
        setSubmitting(false);
        return;
      }
      ...
      if (err) { setError(err.message); } else {
        if (list.length === 0) {
          await supabase.from('collaborators').insert({ email, role: 'admin' });
        }
        setError('');
      }
    } else {
