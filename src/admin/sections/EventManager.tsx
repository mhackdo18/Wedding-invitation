...
function EventForm({ venues, parentId, event, onCancel, onSave }: {
  ...
  const handlePhoto = async (file: File) => {
    ...
    setPhotoProgress(0);
    const url = await uploadImage(file, 'events', setPhotoProgress);
      ...
      <div className="admin-card w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div onClick={() => !photoUploading && fileRef.current?.click()} className={`rounded-lg border-2 border-dashed p-2.5 text-center transition ${photoUploading ? '' : 'cursor-pointer hover:border-[#8a6d3b]'}`} style={{ borderColor: '#d6cdbf', background: '#faf6ee' }}>
            ...
            {photoUploading && <UploadProgress percent={photoProgress} />}
