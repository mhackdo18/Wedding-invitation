...
import { Shield, Users, Plus, Loader2, Trash2, X, Lock, Unlock, Mail, Upload, Image as ImageIcon, Sparkles, Globe } from 'lucide-react';
...
export default function Settings() {
  ...
  const [uploadingThankYouHero, setUploadingThankYouHero] = useState(false);
  const [thankYouHeroProgress, setThankYouHeroProgress] = useState(0);
  const monogramRef = useRef<HTMLInputElement>(null);
  const thankYouHeroRef = useRef<HTMLInputElement>(null);
  ...
  const saveSettings = async () => {
    ...
    await supabase.from('site_settings').update({
      ...
      rsvp_thank_you_title: settings.rsvp_thank_you_title,
      rsvp_thank_you_message: settings.rsvp_thank_you_message,
      rsvp_thank_you_font: settings.rsvp_thank_you_font,
      rsvp_thank_you_font_size: settings.rsvp_thank_you_font_size,
      rsvp_thank_you_text_color: settings.rsvp_thank_you_text_color,
      ...
      scroll_animation_enabled: settings.scroll_animation_enabled,
  ...
  const uploadThankYouHero = async (file: File) => {
    if (uploadingThankYouHero) return;
    setUploadingThankYouHero(true);
    setThankYouHeroProgress(0);
    const url = await uploadImage(file, 'rsvp-hero', setThankYouHeroProgress);
    if (url) updateSettings({ rsvp_hero_image_url: url });
    setUploadingThankYouHero(false);
    setThankYouHeroProgress(0);
  };
  ...
  return (
    <div>
      ...
      <Card className="mb-4">
        <h3 className="font-semibold text-[#3a2e22] mb-3 flex items-center gap-2"><Globe size={16} className="text-[#8a6d3b]" /> Timezone</h3>
        <p className="text-xs text-[#8a7a66] mb-3">All RSVP submission timestamps in the guest list will be displayed in this timezone so every guest's time is consistent regardless of where they submitted from.</p>
        <select className="admin-input" value={settings?.timezone || 'UTC'} onChange={(e) => updateSettings({ timezone: e.target.value })}>
          {getTimezoneOptions().map((tz) => <option key={tz.value} value={tz.value}>{tz.label}</option>)}
        </select>
      </Card>

      <Card className="mb-4">
        <h3 className="font-semibold text-[#3a2e22] mb-3 flex items-center gap-2"><Sparkles size={16} className="text-[#8a6d3b]" /> Scroll Animations</h3>
        <p className="text-xs text-[#8a7a66] mb-3">When enabled, text and items on the public site and guest portal will gently animate into view as guests scroll down the page.</p>
        <label className="flex items-center gap-2 text-sm text-[#5a4430]">
          <input type="checkbox" checked={settings?.scroll_animation_enabled ?? true} onChange={(e) => updateSettings({ scroll_animation_enabled: e.target.checked })} className="accent-[#8a6d3b]" />
          <span>{settings?.scroll_animation_enabled ? 'Animations enabled' : 'Animations disabled'}</span>
        </label>
      </Card>
      ...
      <Card className="mb-4">
        <h3 className="font-semibold text-[#3a2e22] mb-3 flex items-center gap-2"><ImageIcon size={16} className="text-[#8a6d3b]" /> RSVP Thank-You Screen</h3>
        <p className="text-xs text-[#8a7a66] mb-3">Customize the confirmation guests see after submitting their RSVP. Upload a photo to replace the monogram; if no photo is set, the monogram is used.</p>
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              {settings?.rsvp_hero_image_url ? (
                <div className="relative">
                  <img src={settings.rsvp_hero_image_url} alt="Thank-you photo" className="w-24 h-16 rounded-lg object-cover border" style={{ borderColor: '#e6ddcd' }} />
                  <button onClick={() => updateSettings({ rsvp_hero_image_url: null })} className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ background: '#b03a3a' }}><X size={10} className="text-white" /></button>
                </div>
              ) : (
                <div className="w-24 h-16 rounded-lg border flex items-center justify-center" style={{ borderColor: '#e6ddcd', background: '#faf6ee' }}>
                  <ImageIcon size={18} className="text-[#c9b896]" />
                </div>
              )}
              <label className={`cursor-pointer ${uploadingThankYouHero ? 'pointer-events-none opacity-50' : ''}`}>
                <span className="btn-ghost inline-flex items-center gap-1.5 text-sm">
                  {uploadingThankYouHero ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />}
                  {settings?.rsvp_hero_image_url ? 'Replace' : 'Upload Photo'}
                </span>
                <input ref={thankYouHeroRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadThankYouHero(f); e.target.value = ''; }} />
              </label>
            </div>
            {uploadingThankYouHero && <UploadProgress percent={thankYouHeroProgress} />}
            <p className="text-xs text-[#8a7a66] mt-1">Leave blank to fall back to the site monogram.</p>
          </div>
          <div>
            <label className="admin-label">Title</label>
            <input type="text" className="admin-input" value={settings?.rsvp_thank_you_title || ''} onChange={(e) => updateSettings({ rsvp_thank_you_title: e.target.value })} placeholder="Thank you" />
          </div>
          <div>
            <label className="admin-label">Message</label>
            <textarea className="admin-input" rows={2} value={settings?.rsvp_thank_you_message || ''} onChange={(e) => updateSettings({ rsvp_thank_you_message: e.target.value })} placeholder="Your RSVP has been received." />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="admin-label">Font</label>
              <FontSelect value={settings?.rsvp_thank_you_font || undefined} onChange={(f) => updateSettings({ rsvp_thank_you_font: f })} previewText="Thank you" previewSize={22} previewColor={settings?.rsvp_thank_you_text_color || '#5a4430'} />
            </div>
            <div>
              <label className="admin-label">Title Size: {settings?.rsvp_thank_you_font_size ?? 30}px</label>
              <input type="range" min={16} max={60} value={settings?.rsvp_thank_you_font_size ?? 30} onChange={(e) => updateSettings({ rsvp_thank_you_font_size: parseInt(e.target.value) })} className="w-full accent-[#8a6d3b]" />
            </div>
          </div>
          <div>
            <label className="admin-label">Text Color</label>
            <div className="flex items-center gap-2">
              <input type="color" value={settings?.rsvp_thank_you_text_color || '#5a4430'} onChange={(e) => updateSettings({ rsvp_thank_you_text_color: e.target.value })} className="w-10 h-10 rounded border cursor-pointer" style={{ borderColor: '#e6ddcd' }} />
              <input type="text" value={settings?.rsvp_thank_you_text_color || ''} onChange={(e) => updateSettings({ rsvp_thank_you_text_color: e.target.value })} placeholder="#5a4430" className="admin-input flex-1" />
            </div>
          </div>
          <div className="rounded-lg p-4 text-center" style={{ background: settings?.bg_color || '#f4efe6' }}>
            <p className="text-[10px] uppercase tracking-wider text-[#a07c4a] mb-2">Preview</p>
            {settings?.rsvp_hero_image_url ? (
              <img src={settings.rsvp_hero_image_url} alt="Preview" className="w-full max-w-xs h-32 object-cover rounded-lg mx-auto mb-3" />
            ) : (
              <div className="w-14 h-14 rounded-full mx-auto flex items-center justify-center mb-3" style={{ background: '#e8dfc8' }}>
                <ImageIcon size={22} className="text-[#a07c4a]" />
              </div>
            )}
            <h3 style={{ fontFamily: stackFor(settings?.rsvp_thank_you_font || 'Cormorant Garamond'), fontSize: settings?.rsvp_thank_you_font_size ?? 30, color: settings?.rsvp_thank_you_text_color || '#5a4430', margin: 0 }}>{settings?.rsvp_thank_you_title || 'Thank you'}</h3>
            <p className="text-sm mt-2" style={{ color: settings?.rsvp_thank_you_text_color || '#6b5d4f', fontFamily: stackFor(settings?.rsvp_thank_you_font || 'Cormorant Garamond') }}>{settings?.rsvp_thank_you_message || 'Your RSVP has been received.'}</p>
          </div>
        </div>
      </Card>
