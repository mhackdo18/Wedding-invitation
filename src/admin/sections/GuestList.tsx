...
import { Users, Plus, Loader2, Trash2, Edit2, X, Search, Download, Upload, Tag, UserPlus, List, Layers, CheckSquare, Square, Settings2 } from 'lucide-react';
...
export default function GuestList() {
  ...
  const filtered = useMemo(() => guests.filter((g) => {
    ...
    return true;
  }), [guests, search, filterStatus, filterTag, filterParty, parties]);
  ...
  const handleImport = async (file: File) => {
    ...
    setImportResult(`Imported ${imported} guest${imported !== 1 ? 's' : ''} successfully.`);
    load();
  };
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b" style={{ borderColor: '#e6ddcd', background: '#faf6ee' }}>
                  <th className="p-2.5 w-8"><button onClick={toggleSelectAll}>{selected.size === filtered.length && filtered.length > 0 ? <CheckSquare size={16} className="text-[#8a6d3b]" /> : <Square size={16} className="text-[#c9b896]" />}</button></th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Name</th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Contact</th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Party</th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Plus One</th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Tags</th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Proxy</th>
                  <th className="text-left p-2.5 font-semibold text-[#6b5d4f] whitespace-nowrap">Status</th>
                  ...
                  for (let i = 0; i < sortedFiltered.length; i++) {
                    ...
                    if (isPartyStart) {
                      rows.push(
                        <tr key={`party-header-${g.party_id}`}>
                          <td colSpan={9} className="px-2.5 pt-3 pb-0">
                    ...
                    rows.push(
                      <tr key={g.id} style={{
                        ...
                        <td className="p-2.5 text-xs text-[#8a7a66] whitespace-nowrap">{g.email || g.phone || '\u2014'}</td>
                        <td className="p-2.5 text-xs text-[#5a4430] whitespace-nowrap">{partyName(g.party_id) || '\u2014'}</td>
                        <td className="p-2.5 text-xs text-[#8a7a66] whitespace-nowrap">{g.plus_one_name || (g.plus_one_allowed ? <span className="text-[#c9b896]">allowed</span> : '\u2014')}</td>
                        <td className="p-2.5"><div className="flex flex-wrap gap-1">{(g.tags || []).map((t) => <span key={t} className="inline-block px-1.5 py-0.5 rounded-full text-[10px] font-semibold whitespace-nowrap" style={{ background: '#f0e8d8', color: '#8a6d3b' }}>{t}</span>)}</div></td>
                        <td className="p-2.5 text-xs text-[#8a7a66] whitespace-nowrap">{g.proxy_guest_name || '\u2014'}</td>
                        <td className="p-2.5"><span className="inline-flex items-center gap-1.5 text-xs font-medium" style={{ color: STATUS_COLORS[g.rsvp_status] }}><span className="w-2 h-2 rounded-full" style={{ background: STATUS_COLORS[g.rsvp_status] }} />{g.rsvp_status}</span></td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(30,20,12,0.4)' }} onClick={onCancel}>
      <div className="admin-card w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-2 max-h-60 overflow-auto thin-scroll">
          {parties.map((p) => <button key={p.id} onClick={() => onAssign(p.id)} className="w-full text-left rounded-lg border p-2.5 text-sm hover:border-[#8a6d3b]" style={{ borderColor: '#e6ddcd', background: '#faf6ee', color: '#5a4430' }}>{p.name}</button>)}
        </div>
...
function GuestForm({ guest, parties, onCancel, onSave }: { guest: Guest | null; parties: Party[]; onCancel: () => void; onSave: (g: Partial<Guest>) => void }) {
  const [form, setForm] = useState<Partial<Guest>>(guest || { name: '', email: '', phone: '', party_size: 1, rsvp_status: 'pending', dietary: '', plus_one_name: '', song_requests: '', notes: '', tags: [], plus_one_allowed: false, is_party_leader: false });
