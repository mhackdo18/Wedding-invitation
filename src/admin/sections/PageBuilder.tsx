...
function PageForm({ page, onCancel, onSave, existingSlugs }: { page: Page | null; onCancel: () => void; onSave: (d: Partial<Page>) => void; existingSlugs: string[] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background: 'rgba(30,20,12,0.4)' }} onClick={onCancel}>
      <div className="admin-card w-full max-w-md p-5 flex flex-col min-h-0 max-h-[90vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <div onClick={() => musicRef.current?.click()} className="rounded-lg border-2 border-dashed p-3 flex items-center gap-3 cursor-pointer transition hover:border-[#8a6d3b]" style={{ borderColor: '#d6cdbf', background: '#faf6ee' }}>
              ...
              <div className="text-sm text-[#6b5d4f]">
                {draft.music_url ? <span><span className="text-[#5a7a4a] font-medium">Music uploaded</span> — click to replace</span> : 'Click to upload background music'}
              </div>
              <input ref={musicRef} type="file" accept="audio/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleMusicUpload(f); }} />
            </div>
            {draft.music_url && (
...
function InformationConfigEditor({ form, setForm }: { form: Partial<Page>; setForm: (f: Partial<Page>) => void }) {
  ...
  const addBlock = () => {
    const newBlock: InformationBlock = { heading: '', body_html: '', photo_url: null };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="p-3">
        ...
        <p className="text-xs text-[#8a7a66] mb-3">Each block has a heading, rich text body, and a photo below the body. All blocks are single column.</p>
        <div className="space-y-3">
          {cfg.blocks.map((block, i) => (
            <div key={i} className="p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-[#6b5d4f]">Block {i + 1}</span>
                <div className="flex items-center gap-1">
                  <button onClick={() => moveBlock(i, -1)} disabled={i === 0} className="p-1 rounded hover:bg-[#f0e8d8] disabled:opacity-30"><ChevronUp size={14} className="text-[#8a7a66]" /></button>
                  <button onClick={() => moveBlock(i, 1)} disabled={i === cfg.blocks.length - 1} className="p-1 rounded hover:bg-[#f0e8d8] disabled:opacity-30"><ChevronDown size={14} className="text-[#8a7a66]" /></button>
                  <ConfirmButton onConfirm={() => removeBlock(i)}><Trash2 size={14} /></ConfirmButton>
                </div>
              </div>
              <div className="space-y-2">
                <div>
                  <label className="admin-label">Heading (rich text)</label>
                  <RichTextEditor value={block.heading} onChange={(html) => updateBlock(i, { heading: html })} placeholder="Section heading..." rows={2} />
                </div>
                <div className="rounded border p-2" style={{ borderColor: '#f0e8d8', background: '#faf6ee' }}>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="cursor-pointer flex items-center gap-2 text-xs text-[#8a6d3b] hover:text-[#5a4430]">
                      {uploading === i ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />}
                      {block.photo_url ? 'Replace photo' : 'Upload photo'}
                      <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadPhoto(i, f); }} />
                    </label>
                    {block.photo_url && (
