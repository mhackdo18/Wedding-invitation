  ...
  return (
    <div className={container}>
      <div className="relative w-full max-w-sm" style={{ minHeight: fullscreen ? 'calc(100svh - 4rem)' : 400 }}>
        ...
        >
          {showSeal && (
            <p className="text-center mb-4 text-xs fade-up" style={{ color: '#c9b896', letterSpacing: '0.12em', fontFamily: `'${headingFont}', serif` }}>
          ...
          <div className="relative w-full" style={{ height: 240 }}>
            ...
            {/* Grain */}
            <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: grainTexture, opacity: envIsLight ? 0.1 : 0.06, mixBlendMode: 'multiply' }} />
            <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: fiberTexture, opacity: envIsLight ? 0.05 : 0.03, mixBlendMode: 'multiply' }} />
            {/* Pocket V */}
            ...
            <div className="absolute inset-x-0 top-0" style={{ height: '100%', perspective: 1450, perspectiveOrigin: '50% 8%' }}>
              <div className="w-full h-full origin-top relative" style={{ transform: flapTransform, transformStyle: 'preserve-3d', transition: phase === 'opening' ? 'transform 1.05s cubic-bezier(0.16, 0.86, 0.28, 1)' : 'transform 1s cubic-bezier(0.22, 0.8, 0.26, 1.08)' }}>
                ...
                <div className="w-full h-full absolute inset-0" style={{ background: envGradient, clipPath: 'polygon(0 0, 100% 0, 50% 58%)', boxShadow: `inset 0 -10px 19px ${withAlpha(envDeepShadow, 0.42)}`, backfaceVisibility: 'hidden' }}>
                  <div className="absolute inset-0 pointer-events-none" style={{ clipPath: 'polygon(0 0, 100% 0, 50% 58%)', backgroundImage: grainTexture, opacity: 0.08, mixBlendMode: 'multiply' }} />
                  {/* Guest name on flap */}
                  {showFlapName && flapNameText && (
                    <div className="absolute inset-x-0 top-[14%] text-center pointer-events-none">
                      <p style={{ fontFamily: `'${nameFont}', cursive`, fontSize: 'clamp(1.1rem, 4vw, 1.6rem)', color: nameColor, lineHeight: 1.2, letterSpacing: '0.01em', opacity: 0.85 }}>
                        {flapNameText}
                      </p>
                    </div>
                  )}
                    ...
                    <div className="relative transition-transform duration-300 group-hover:scale-110 group-active:scale-95" style={{ width: 72, height: 72 }}>
                      ...
                      <div className="absolute inset-[10px] rounded-full flex items-center justify-center overflow-hidden" style={{ boxShadow: 'inset 0 5px 10px rgba(0,0,0,0.55)', background: `radial-gradient(circle at 42% 36%, ${waxBase} 0%, ${waxDark} 100%)` }}>
                        {settings.invitation_wax_seal_image_url ? (
                          ...
                          <span style={{ color: 'rgba(255,255,255,0.75)', fontSize: 20, fontFamily: `'${nameFont}', cursive`, textShadow: '0 1px 2px rgba(0,0,0,0.5)' }}>{initials}</span>
                        )}
                      </div>
                      {/* Glint */}
                      <div className="absolute top-2 left-4 w-8 h-3.5 rounded-full opacity-30 pointer-events-none" style={{ background: 'radial-gradient(ellipse, rgba(255,255,255,0.8), transparent 70%)', filter: 'blur(2px)' }} />
                    </div>
                  </button>
                </div>
              </div>
        ...
        <div className="w-full" style={{
          ...
          <div className="overflow-hidden flex flex-col" style={{ maxHeight: fullscreen ? 'calc(100svh - 6rem)' : 380, background: paperBg, border: `1px solid ${paperBorderColor}`, borderRadius: 4, boxShadow: '0 24px 48px rgba(25,18,10,0.22), 0 5px 12px rgba(25,18,10,0.12)' }}>
            <div className="flex-1 min-h-0 p-6 flex flex-col text-center overflow-y-auto items-center thin-scroll">
              ...
              <h2 className="leading-tight mb-1 w-full" style={{ fontFamily: `'${headingFont}', cursive`, fontSize: 'clamp(1.5rem, 5vw, 2.2rem)', color: paperText }}>{settings.partner1_name}</h2>
              <p className="text-xl italic mb-1" style={{ fontFamily: `'${headingFont}', cursive`, color: paperText }}>&amp;</p>
              <h2 className="leading-tight mb-4 w-full" style={{ fontFamily: `'${headingFont}', cursive`, fontSize: 'clamp(1.5rem, 5vw, 2.2rem)', color: paperText }}>{settings.partner2_name}</h2>
              {weddingDate && (
              ...
              <div className="text-sm mb-4 leading-relaxed w-full text-left" style={{ fontFamily: `'${bodyFont}', serif`, color: paperText, opacity: 0.85, whiteSpace: 'pre-wrap' }}>
                {bodyText}
              </div>
              {buttons.length > 0 && (
