...
function addToCalendar(ev: WeddingEvent) {
...
export default function Schedule({
  events, typo,
}: { events: WeddingEvent[]; typo: Record<string, TypeStyle> }) {
  ...
  return (
    <section className="px-6 py-8">
      <div className="text-center mb-5">
        <h2 style={{ ...typeStyle(typo.scheduleTitle), fontFamily: 'var(--heading-font)', fontSize: 26, color: '#5a4430', margin: 0 }}>
          Order of Events
        </h2>
      </div>
          ...
          <div key={group.date}>
            {hasMultipleDates && group.dateLabel && (
              <div className="flex items-center gap-3 mb-4">
                <div className="flex-1 h-px" style={{ background: '#d6cdbf' }} />
                <span style={{ fontSize: 13, fontWeight: 600, color: '#8a6d3b', letterSpacing: '0.03em', whiteSpace: 'nowrap' }}>
                  {group.dateLabel}
                </span>
                <div className="flex-1 h-px" style={{ background: '#d6cdbf' }} />
              </div>
            ...
            {!hasMultipleDates && group.dateLabel && (
              <p className="text-center mb-4" style={{ fontSize: 14, color: '#8a6d3b', fontWeight: 500 }}>{group.dateLabel}</p>
            )}

            <div className="relative">
              <div className="absolute left-3 top-0 bottom-0 w-px" style={{ background: '#d6cdbf' }} />
                ...
                {group.events.map((ev) => {
                  ...
                  return (
                    <div key={ev.id} className="relative pl-10">
                      <div className="absolute left-1.5 top-1.5 w-3 h-3 rounded-full" style={{ background: '#b5462f', border: '2px solid var(--page-color)' }} />
                      <div className="flex items-baseline justify-between gap-3">
                        <h3 style={{ ...typeStyle(typo.eventName), fontFamily: 'var(--heading-font)', fontSize: 20, color: '#5a4430', margin: 0 }}>
                          {ev.title}
                        </h3>
                        {ev.start_time && (
                          <span style={{ ...typeStyle(typo.eventTime), fontSize: 14, color: '#a07c4a', whiteSpace: 'nowrap', fontWeight: 600 }}>
                      ...
                      {ev.description && (
                        <p style={{ fontSize: 13, color: '#8a7a66', marginTop: 4 }}>{ev.description}</p>
                      )}
                        ...
                        <div className="mt-2 rounded-lg overflow-hidden" style={{ background: 'rgba(138,109,59,0.06)', border: '1px solid rgba(138,109,59,0.12)' }}>
                          ...
                          <div className="px-3 py-2 flex items-center gap-1.5">
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="text-xs font-semibold" style={{ color: '#5a4430' }}>{ev.venue.name}</span>
                              {ev.venue.address && <span className="text-xs" style={{ color: '#8a7a66' }}> · {ev.venue.address}</span>}
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 mt-2.5">
                        <button onClick={() => addToCalendar(ev)} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium transition hover:opacity-80" style={{ background: 'rgba(138,109,59,0.12)', color: '#8a6d3b' }}>
                      ...
                      {ev.sub_events && ev.sub_events.length > 0 && (
                        <div className="mt-3 space-y-1.5">
                          {ev.sub_events.map((sub) => (
                            <div key={sub.id} className="flex items-center justify-between gap-2 pl-3 border-l-2" style={{ borderColor: '#e0d4be' }}>
                              <div className="flex items-center gap-1.5 min-w-0">
                                <Clock size={11} style={{ color: '#c9b896' }} />
                                <span style={{ ...typeStyle(typo.subEventName), fontSize: 14, color: '#6b5d4f' }} className="truncate">
                                  {sub.title}
                                </span>
                              </div>
                              <span style={{ fontSize: 12, color: '#a07c4a', whiteSpace: 'nowrap' }}>
