...
export default function Gallery({
  photos, typo, heroImageUrl, animEnabled,
}: { photos: GalleryPhoto[]; typo: Record<string, TypeStyle>; heroImageUrl?: string; animEnabled?: boolean }) {
  ...
  return (
    <section className="px-6 py-8">
      ...
      <HeroImage url={heroImageUrl || ''} alt="Gallery" />
          ...
          <div key={layout} className="mb-6">
            {layout === 'masonry' && (
              <div className="columns-2 gap-2 [&>*]:mb-2">
                {group.map((p, idx) => (
                  <Reveal key={p.id} enabled={!!animEnabled} animation="scale-in" delay={idx * 50} className="block w-full break-inside-avoid">
                  <button onClick={() => setLightbox(photos.indexOf(p))} className="block w-full overflow-hidden rounded-lg">
            ...
            {layout === 'grid' && (
              <div className="grid grid-cols-3 gap-2">
                {group.map((p, idx) => (
                  <Reveal key={p.id} enabled={!!animEnabled} animation="scale-in" delay={idx * 40} className="aspect-square overflow-hidden rounded-lg">
                    <button onClick={() => setLightbox(photos.indexOf(p))} className="w-full h-full">
