  ...
  return (
    <section className="px-6 py-8">
      <div className="text-center mb-5">
        <h2 style={{ ...typeStyle(typo.galleryTitle), fontFamily: 'var(--heading-font)', fontSize: 26, color: '#5a4430', margin: 0 }}>
          Gallery
        </h2>
      </div>
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(30,20,12,0.9)' }} onClick={() => setLightbox(null)}>
          <button className="absolute top-4 right-4 text-white/80" onClick={(e) => { e.stopPropagation(); setLightbox(null); }}><X size={28} /></button>
          <button className="absolute left-3 text-white/70" onClick={(e) => { e.stopPropagation(); prev(); }}><ChevronLeft size={36} /></button>
          <img src={photos[lightbox].image_url} alt="" className="max-w-[90vw] max-h-[80vh] rounded-lg" onClick={(e) => e.stopPropagation()} />
          <button className="absolute right-3 text-white/70" onClick={(e) => { e.stopPropagation(); next(); }}><ChevronRight size={36} /></button>
        </div>
      )}
    </section>
  );
