...
export default function PasswordGate({ settings, onGranted }: { settings: SiteSettings; onGranted: () => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6" style={{ background: settings.bg_color || '#f4efe6' }}>
      <div className="embossed-card rounded-xl p-8 max-w-sm w-full text-center">
        <div className="w-14 h-14 rounded-full mx-auto flex items-center justify-center mb-4" style={{ background: '#f0e8d8' }}>
          <Lock size={24} style={{ color: '#8a6d3b' }} />
        </div>
        <h1 style={{ fontFamily: 'var(--heading-font)', fontSize: 28, color: '#5a4430', margin: '0 0 4px' }}>
          {settings.partner1_name} &amp; {settings.partner2_name}
        </h1>
        <p style={{ fontSize: 13, color: '#8a7a66', marginBottom: 20 }}>
        ...
        <form onSubmit={submit} className="space-y-3">
          <input
            type="password"
            autoFocus
            className="w-full rounded-lg border px-4 py-3 text-sm bg-white/80 outline-none focus:border-[#b59a6b] text-center"
        ...
        <Heart size={14} className="mx-auto mt-6" style={{ color: '#b5462f' }} />
      </div>
    </div>
  );
}
