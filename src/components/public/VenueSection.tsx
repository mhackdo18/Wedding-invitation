      ...
      <div className="max-w-md mx-auto space-y-4">
        {venues.map((v) => (
          <div key={v.id} className="rounded-xl overflow-hidden" style={{ background: '#fff', border: '1px solid #e6ddcd' }}>
            ...
            <div className="p-4">
              <h3 style={{ fontFamily: 'var(--heading-font)', fontSize: 20, color: '#5a4430', margin: 0 }}>
                {v.name}
              </h3>
              {v.address && (
                <p className="flex items-center gap-1.5 mt-1.5" style={{ fontSize: 13, color: '#8a7a66' }}>
                  <MapPin size={13} /> {v.address}
                </p>
              )}
              {v.description && (
                <p style={{ fontSize: 13, lineHeight: 1.6, color: '#6b5d4f', marginTop: 6 }}>
                  {v.description}
                </p>
              )}
