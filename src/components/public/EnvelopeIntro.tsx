...
export default function EnvelopeIntro({
  ...
  const fillLetterBody = (body: string) =>
    body
  ...
  // --- Envelope colors ---
  const env = settings.env_color || '#FAFAFA';
  ...
  // --- Wax seal colors ---
  const waxBase = settings.seal_color || '#C5A059';
  ...
  const letterFontStack = stackFor(settings.letter_font);
  const weddingDate = settings.wedding_date ? new Date(settings.wedding_date) : null;

  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center px-3 py-5 sm:px-4 sm:py-8 overflow-x-hidden" style={{ background: 'rgba(40,28,18,0.82)', backdropFilter: 'blur(6px)' }}>
              ...
              >
                ...
                <div className="w-full h-full absolute inset-0" style={{
                  ...
                  <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 100 100" preserveAspectRatio="none" style={{ overflow: 'visible' }}>
                    ...
                    <line x1="0" y1="0" x2="50" y2="58" stroke={envIsLight ? 'rgba(70,70,70,0.5)' : 'rgba(0,0,0,0.5)'} strokeWidth="0.5" vectorEffect="non-scaling-stroke" filter="url(#flapCreaseShadow)" />
                    <line x1="100" y1="0" x2="50" y2="58" stroke={envIsLight ? 'rgba(70,70,70,0.5)' : 'rgba(0,0,0,0.5)'} strokeWidth="0.5" vectorEffect="non-scaling-stroke" filter="url(#flapCreaseShadow)" />
                  </svg>
                </div>
                    ...
                    <div className="relative transition-transform duration-300 group-hover:scale-110 group-active:scale-95" style={{ width: 'clamp(4.75rem, 23vw, 6rem)', height: 'clamp(4.75rem, 23vw, 6rem)' }}>
                      ...
                      <div className="absolute inset-[10px] rounded-full flex items-center justify-center overflow-hidden" style={{
                        ...
                        {settings.monogram_url ? (
                          ...
                          <span style={{ color: 'rgba(255,255,255,0.75)', textShadow: '0 1px 2px rgba(0,0,0,0.5), 0 -1px 1px rgba(255,255,255,0.12)', fontSize: 24, fontFamily: "'Great Vibes', cursive" }}>
        ...
        >
          <div className="overflow-hidden flex flex-col h-[calc(100svh-5rem)] sm:h-auto" style={{
            ...
            <div className="flex-1 min-h-0 p-6 sm:p-10 flex flex-col text-center overflow-y-auto items-center w-full">
              ...
              {/* Partner names */}
              <h2 className="leading-tight mb-1 w-full" style={{ fontFamily: "'Great Vibes', cursive", fontSize: 'clamp(1.8rem, 6vw, 2.5rem)', color: '#5a4430' }}>
                {settings.partner1_name}
              </h2>
              <p className="text-xl italic mb-1" style={{ fontFamily: "'Great Vibes', cursive", color: '#8a6d3b' }}>&amp;</p>
              <h2 className="leading-tight mb-4 w-full" style={{ fontFamily: "'Great Vibes', cursive", fontSize: 'clamp(1.8rem, 6vw, 2.5rem)', color: '#5a4430' }}>
              ...
              {/* Letter body */}
              <div className="text-sm mb-6 leading-relaxed w-full" style={{ fontFamily: letterFontStack, color: '#5a4430', opacity: 0.85, whiteSpace: 'pre-wrap' }}>
                {fillLetterBody(settings.letter_body || `Dear ${displayName},\n\nWe would be honoured to have you join us as we celebrate our union. Your presence would mean the world to us as we begin this new chapter together.\n\nWith love,\n${settings.partner1_name} & ${settings.partner2_name}`)}
              </div>
